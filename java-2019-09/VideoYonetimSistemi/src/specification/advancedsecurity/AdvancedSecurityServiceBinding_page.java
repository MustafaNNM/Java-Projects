...
public class AdvancedSecurityServiceBinding_page extends Page{
	...
	public AdvancedSecurityServiceBinding_page(String target) {
		super(target);
		requestURI = "";
	...
	public void load(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) {
		target = target.substring(Environment.advancedsecurityservicebinding_page.getSize());
		if (target.equals("/AdvancedSecurityServiceBinding")) {
			AdvancedSecurityServiceBinding.GetServiceCapabilities(target, baseRequest, request, response);
		} else {
			load_not_found_page(
					target,
					baseRequest,
