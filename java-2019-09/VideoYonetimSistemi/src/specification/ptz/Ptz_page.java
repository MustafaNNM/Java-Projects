 ...
 *
 */
public class Ptz_page extends Page{
	...
	public void load(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) {
		System.out.println("Ptz_page loading");
		/* request uri starts with Environment.ptz_page.getURI() */
		if (target != null) {
			if (target.equals("")) {
				/* request uri equals Environment.ptz_page.getURI() */
				...
				if (target.startsWith(Environment.ptzbinding_page.getURI())) {
					load_PTZBinding_page(target, baseRequest, request, response);
				} else {
					/* there is no other binding in ptz */
	...
	private void load_PTZBinding_page(String target, Request baseRequest, HttpServletRequest request,
