...
public class HelloWorld extends AbstractHandler {
	public void handle(String target, Request baseRequest,
		...
		if (requestURI.startsWith("/img/")) {
			...
			String siteUrl = "https://drive.google.com/open?id=0B7o_JE2gTE3CVFNWWVJQR3kzeVU";
			String image_java_logo = "avajavalogo.jpg";
			//image_name = image_java_logo;
			Services services = new Services();
			services.saveImage(siteUrl, image_java_logo,response.getOutputStream());
			//services.saveImage(imageUrl, image_name,response.getOutputStream());

			System.out.println("DEBUG:6");
			if (image_name.length() > 0) {

				services.resmi_oku(siteUrl, image_name);
						...
						if (is_IMG_COLOR_received) {
							...
							if (f.exists() && f.isFile()) {
								...
								resizedImage = services.scalar.resizeImage(
										originalImage, IMG_WIDTH, IMG_HEIGHT);
								// img klasörüne kaydedilmesi gerekiyor
								System.out.println("DEBUG:95");
								ImageIO.write(resizedImage, "jpg",
										response.getOutputStream());
								// ilk biti bir an önce göndermek lazým
