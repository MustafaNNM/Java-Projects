...
void gun_listele(struct mesai *bas)
{
    int sa,dk,top_dk=0,top_sa=0,ort_sa,sayac=0;
    //sa: saat dk:dakika top_dk:toplam dakika ort_sa:ortalama saat

    while (bas!=NULL)
    {
        dk=((bas->cikis_saati-bas->giris_saati)*60+bas->cikis_dakikasi-bas->giris_dakikasi)%60;
        sa=((bas->cikis_saati-bas->giris_saati)*60+bas->cikis_dakikasi-bas->giris_dakikasi)/60;
        top_dk+=dk;
        top_sa+=sa;
        sayac++;
